...
public class MipsGenModule {
    ...
    public void genMips() throws IOException {
        IOUtils.mips(".data\n");
    ...
    private void getSpArray(String name, int offset, Value value) throws IOException {
        if (mem.containsKey(name)) {
            return;
        }
        getSp(name, value);
    ...
    private void translate(Instruction ir) throws IOException {
    ...
    private void parseBinary(BinaryInst b) throws IOException {
    ...
    private void optimizeMul(Value operand, ConstInt immValue, Instruction b, boolean isMod) throws IOException {
    ...
    private void optimizeDiv(Value operand, ConstInt immValue, BinaryInst b, boolean isMod) throws IOException {
        int imm = immValue.getValue();
        if (imm == 1) {
            load("$t0", operand.getUniqueName());
        } else if (imm == -1) {
    ...
    private void optimizeMod(Value operand, ConstInt immValue, BinaryInst b) throws IOException {
        int imm = immValue.getValue();
        if (imm == 1 || imm == -1) {
            load("$t0", "0");
        } else {
    ...
    private void calc(BinaryInst b, String op, int type) throws IOException {
    ...
    private void parseCall(CallInst callInst) throws IOException {
    ...
    private void parseRet(RetInst ret) throws IOException {
        if (!ret.isVoid()) {
    ...
    private void parseAlloca(AllocaInst allocaInst) throws IOException {
    ...
    private void parseLoad(LoadInst loadInst) throws IOException {
    ...
    private void parseStore(StoreInst storeInst) throws IOException {
    ...
    private void parseGEP(GEPInst gepInst) throws IOException {
    ...
    private void parseBr(BrInst brInst) throws IOException {
        if (brInst.isCondBr()) {
    ...
    private void parseConv(ConvInst convInst) throws IOException {
    ...
    private void load(String reg, String name) throws IOException {
        if (isNumber(name)) {
    ...
    private void load(String reg, String name, int offset) throws IOException {
        IOUtils.mips("lw " + reg + ", " + offset + "(" + name + ")\n");
    }

    private void store(String reg, String name) throws IOException {
    ...
    private void store(String reg, String name, int offset) throws IOException {
